...
// Give the value of i which vector you have to test
   predictlabel = svm_classify(testData,supportVectors,shift,scale,model_bias,alphas,20);
        for(i = 0; i < testsize; ++i)
        {
                if (predictlabel==testData[i][4])
                {
                        count++;
                }
        }
        printf("Predict Label: %f\n",predictlabel);
        printf("Counter for predict label: %d\n", count);
return 0;
}
